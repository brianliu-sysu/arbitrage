...
// PancakePoolRepository persists PancakeSwap V3 pool aggregate state in PostgreSQL.
type PancakePoolRepository struct {
	db *DB
}

func NewPancakePoolRepository(db *DB) *PancakePoolRepository {
	return &PancakePoolRepository{db: db}
}
...
func (r *PancakePoolRepository) Save(ctx context.Context, pool *marketpancake.Pool) error {
	...
	if err := upsertPancakePool(ctx, tx, pool); err != nil {
		return err
	}
	if err := replacePancakePoolTicks(ctx, tx, pool); err != nil {
		return err
	}
	if err := replacePancakePoolBitmap(ctx, tx, pool); err != nil {
		return err
	}
...
func (r *PancakePoolRepository) Get(ctx context.Context, address common.Address) (*marketpancake.Pool, error) {
	...
	if err := row.Scan(
		...
		return nil, fmt.Errorf("scan pancake pool: %w", err)
	}

	ticks, err := loadPancakePoolTicks(ctx, r.db.pool, address)
	if err != nil {
		return nil, err
	}
	bitmap, err := loadPancakePoolBitmap(ctx, r.db.pool, address)
	if err != nil {
		return nil, err
	}

	pool := marketpancake.NewPool(address, codec.BytesToAddress(token0), codec.BytesToAddress(token1), fee, tickSpacing)
...
func (r *PancakePoolRepository) Delete(ctx context.Context, address common.Address) error {
	_, err := r.db.pool.Exec(ctx, `DELETE FROM pancake_pools WHERE address = $1`, codec.AddressToBytes(address))
	if err != nil {
		return fmt.Errorf("delete pancake pool: %w", err)
	}
	return nil
}
...
func (r *PancakePoolRepository) AdvanceSyncProgressMany(ctx context.Context, addresses []common.Address, blockNumber uint64) error {
	...
	if len(addresses) == 1 {
		return r.advancePancakeSyncProgressOne(ctx, addresses[0], blockNumber)
	}
	...
	if err != nil {
		return fmt.Errorf("advance pancake sync progress: %w", err)
	}
	if tag.RowsAffected() != int64(len(addresses)) {
		return fmt.Errorf("expected to update %d pancake pools, updated %d", len(addresses), tag.RowsAffected())
	}
	return nil
}

func (r *PancakePoolRepository) advancePancakeSyncProgressOne(ctx context.Context, address common.Address, blockNumber uint64) error {
	tag, err := r.db.pool.Exec(ctx, `
		UPDATE pancake_pools SET
			last_block_number = GREATEST(last_block_number, $2),
			status = CASE WHEN status = $3 THEN $4 ELSE status END,
	...
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("pancake pool %s not found", address.Hex())
	}
	return nil
}

func upsertPancakePool(ctx context.Context, tx pgx.Tx, pool *marketpancake.Pool) error {
	_, err := tx.Exec(ctx, `
		INSERT INTO pancake_pools (
	...
	if err != nil {
		return fmt.Errorf("upsert pancake pool: %w", err)
	}
	return nil
}

func replacePancakePoolTicks(ctx context.Context, tx pgx.Tx, pool *marketpancake.Pool) error {
	if _, err := tx.Exec(ctx, `DELETE FROM pancake_pool_ticks WHERE pool_address = $1`, codec.AddressToBytes(pool.Address)); err != nil {
		return fmt.Errorf("delete pancake pool ticks: %w", err)
	}
	for _, tick := range pool.Ticks.ExportTicks() {
		if _, err := tx.Exec(ctx, `
			INSERT INTO pancake_pool_ticks (pool_address, tick_index, liquidity_gross, liquidity_net)
			VALUES ($1, $2, $3, $4)
		`,
			codec.AddressToBytes(pool.Address),
			tick.Index,
			...
			return fmt.Errorf("insert pancake pool tick: %w", err)
		}
	}
	return nil
}

func replacePancakePoolBitmap(ctx context.Context, tx pgx.Tx, pool *marketpancake.Pool) error {
	if _, err := tx.Exec(ctx, `DELETE FROM pancake_pool_tick_bitmap WHERE pool_address = $1`, codec.AddressToBytes(pool.Address)); err != nil {
		return fmt.Errorf("delete pancake pool bitmap: %w", err)
	}
	for _, word := range pool.Bitmap.ExportBitmap() {
		if _, err := tx.Exec(ctx, `
			INSERT INTO pancake_pool_tick_bitmap (pool_address, word_pos, word_value)
			VALUES ($1, $2, $3)
		`,
			codec.AddressToBytes(pool.Address),
			word.WordPos,
			...
			return fmt.Errorf("insert pancake pool bitmap word: %w", err)
		}
	}
	return nil
}

func loadPancakePoolTicks(ctx context.Context, pool *pgxpool.Pool, address common.Address) (market.TickTable, error) {
	rows, err := pool.Query(ctx, `
		SELECT tick_index, liquidity_gross::text, liquidity_net::text
		FROM pancake_pool_ticks
		WHERE pool_address = $1
	`, codec.AddressToBytes(address))
	if err != nil {
		return market.TickTable{}, fmt.Errorf("query pancake pool ticks: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&index, &gross, &net); err != nil {
			return market.TickTable{}, fmt.Errorf("scan pancake pool tick: %w", err)
		}
...
func loadPancakePoolBitmap(ctx context.Context, pool *pgxpool.Pool, address common.Address) (market.TickBitmap, error) {
	rows, err := pool.Query(ctx, `
		SELECT word_pos, word_value::text
		FROM pancake_pool_tick_bitmap
		WHERE pool_address = $1
	`, codec.AddressToBytes(address))
	if err != nil {
		return market.TickBitmap{}, fmt.Errorf("query pancake pool bitmap: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&wordPos, &wordValue); err != nil {
			return market.TickBitmap{}, fmt.Errorf("scan pancake pool bitmap: %w", err)
		}
