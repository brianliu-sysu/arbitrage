...
// QuickSwapPoolRepository persists QuickSwap V3 pool aggregate state in PostgreSQL.
type QuickSwapPoolRepository struct {
	db *DB
}

func NewQuickSwapPoolRepository(db *DB) *QuickSwapPoolRepository {
	return &QuickSwapPoolRepository{db: db}
}
...
func (r *QuickSwapPoolRepository) Save(ctx context.Context, pool *marketquick.Pool) error {
	...
	if err := upsertQuickSwapPool(ctx, tx, pool); err != nil {
		return err
	}
	if err := replaceQuickSwapPoolTicks(ctx, tx, pool); err != nil {
		return err
	}
	if err := replaceQuickSwapPoolBitmap(ctx, tx, pool); err != nil {
		return err
	}
...
func (r *QuickSwapPoolRepository) Get(ctx context.Context, address common.Address) (*marketquick.Pool, error) {
	...
	if err := row.Scan(
		...
		return nil, fmt.Errorf("scan quick pool: %w", err)
	}

	ticks, err := loadQuickSwapPoolTicks(ctx, r.db.pool, address)
	if err != nil {
		return nil, err
	}
	bitmap, err := loadQuickSwapPoolBitmap(ctx, r.db.pool, address)
	if err != nil {
		return nil, err
	}

	pool := marketquick.NewPool(address, codec.BytesToAddress(token0), codec.BytesToAddress(token1), fee, tickSpacing)
...
func (r *QuickSwapPoolRepository) Delete(ctx context.Context, address common.Address) error {
	_, err := r.db.pool.Exec(ctx, `DELETE FROM quickswap_pools WHERE address = $1`, codec.AddressToBytes(address))
	if err != nil {
		return fmt.Errorf("delete quick pool: %w", err)
	}
	return nil
}
...
func (r *QuickSwapPoolRepository) AdvanceSyncProgressMany(ctx context.Context, addresses []common.Address, blockNumber uint64) error {
	...
	if len(addresses) == 1 {
		return r.advanceQuickSwapSyncProgressOne(ctx, addresses[0], blockNumber)
	}
	...
	if err != nil {
		return fmt.Errorf("advance quick sync progress: %w", err)
	}
	if tag.RowsAffected() != int64(len(addresses)) {
		return fmt.Errorf("expected to update %d quick pools, updated %d", len(addresses), tag.RowsAffected())
	}
	return nil
}

func (r *QuickSwapPoolRepository) advanceQuickSwapSyncProgressOne(ctx context.Context, address common.Address, blockNumber uint64) error {
	tag, err := r.db.pool.Exec(ctx, `
		UPDATE quickswap_pools SET
			last_block_number = GREATEST(last_block_number, $2),
			status = CASE WHEN status = $3 THEN $4 ELSE status END,
	...
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("quick pool %s not found", address.Hex())
	}
	return nil
}

func upsertQuickSwapPool(ctx context.Context, tx pgx.Tx, pool *marketquick.Pool) error {
	_, err := tx.Exec(ctx, `
		INSERT INTO quickswap_pools (
	...
	if err != nil {
		return fmt.Errorf("upsert quick pool: %w", err)
	}
	return nil
}

func replaceQuickSwapPoolTicks(ctx context.Context, tx pgx.Tx, pool *marketquick.Pool) error {
	if _, err := tx.Exec(ctx, `DELETE FROM quickswap_pool_ticks WHERE pool_address = $1`, codec.AddressToBytes(pool.Address)); err != nil {
		return fmt.Errorf("delete quick pool ticks: %w", err)
	}
	for _, tick := range pool.Ticks.ExportTicks() {
		if _, err := tx.Exec(ctx, `
			INSERT INTO quickswap_pool_ticks (pool_address, tick_index, liquidity_gross, liquidity_net)
			VALUES ($1, $2, $3, $4)
		`,
			codec.AddressToBytes(pool.Address),
			tick.Index,
			...
			return fmt.Errorf("insert quick pool tick: %w", err)
		}
	}
	return nil
}

func replaceQuickSwapPoolBitmap(ctx context.Context, tx pgx.Tx, pool *marketquick.Pool) error {
	if _, err := tx.Exec(ctx, `DELETE FROM quickswap_pool_tick_bitmap WHERE pool_address = $1`, codec.AddressToBytes(pool.Address)); err != nil {
		return fmt.Errorf("delete quick pool bitmap: %w", err)
	}
	for _, word := range pool.Bitmap.ExportBitmap() {
		if _, err := tx.Exec(ctx, `
			INSERT INTO quickswap_pool_tick_bitmap (pool_address, word_pos, word_value)
			VALUES ($1, $2, $3)
		`,
			codec.AddressToBytes(pool.Address),
			word.WordPos,
			...
			return fmt.Errorf("insert quick pool bitmap word: %w", err)
		}
	}
	return nil
}

func loadQuickSwapPoolTicks(ctx context.Context, pool *pgxpool.Pool, address common.Address) (market.TickTable, error) {
	rows, err := pool.Query(ctx, `
		SELECT tick_index, liquidity_gross::text, liquidity_net::text
		FROM quickswap_pool_ticks
		WHERE pool_address = $1
	`, codec.AddressToBytes(address))
	if err != nil {
		return market.TickTable{}, fmt.Errorf("query quick pool ticks: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&index, &gross, &net); err != nil {
			return market.TickTable{}, fmt.Errorf("scan quick pool tick: %w", err)
		}
...
func loadQuickSwapPoolBitmap(ctx context.Context, pool *pgxpool.Pool, address common.Address) (market.TickBitmap, error) {
	rows, err := pool.Query(ctx, `
		SELECT word_pos, word_value::text
		FROM quickswap_pool_tick_bitmap
		WHERE pool_address = $1
	`, codec.AddressToBytes(address))
	if err != nil {
		return market.TickBitmap{}, fmt.Errorf("query quick pool bitmap: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&wordPos, &wordValue); err != nil {
			return market.TickBitmap{}, fmt.Errorf("scan quick pool bitmap: %w", err)
		}
