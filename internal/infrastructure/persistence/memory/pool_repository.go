...
func NewPoolRepository() *PoolRepository {
	return &PoolRepository{pools: make(map[common.Address]*marketv3.Pool)}
}

func (r *PoolRepository) Save(_ context.Context, pool *marketv3.Pool) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PoolRepository) Get(_ context.Context, address common.Address) (*marketv3.Pool, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
