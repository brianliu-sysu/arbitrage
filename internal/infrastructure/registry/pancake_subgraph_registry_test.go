...
func TestPancakeSubgraphRegistryBuildDailySnapshotQuery(t *testing.T) {
	registry := NewPancakeSubgraphRegistry(config.SubgraphPoolConfig{
		Enabled:                true,
	...
	if mode != pancakeQueryModeDailySnapshot {
...
func TestPancakePoolMatchesFeeTiers(t *testing.T) {
	pool := pancakeSubgraphPool{
		ID: "0x1",
		Fees: []pancakeSubgraphFee{
			{FeeType: "FIXED_PROTOCOL_FEE", FeePercentage: "0.0033"},
			{FeeType: "FIXED_TRADING_FEE", FeePercentage: "0.05"},
		},
	}
	if !pancakePoolMatchesFeeTiers(pool, []uint32{500}) {
		t.Fatal("expected 0.05% trading fee to match tier 500")
	}
	if pancakePoolMatchesFeeTiers(pool, []uint32{2500}) {
		t.Fatal("expected tier 2500 to not match 0.05% trading fee")
	}
}

func TestPancakeSubgraphRegistryBuildLiquidityPoolsQuery(t *testing.T) {
	registry := NewPancakeSubgraphRegistry(config.SubgraphPoolConfig{
		Enabled:                true,
		Endpoint:               "http://example.com",
	...
	if mode != pancakeQueryModeLiquidityPools {
