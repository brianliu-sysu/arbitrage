...
// Pool is the aggregate root for PancakeSwap V3 market state.
type Pool struct {
	clv3.Pool
}
...
func (p *Pool) Ref() market.PoolRef {
	return market.PoolRefFromPancakeV3(p.Address)
}

// PoolRegistry defines which PancakeSwap V3 pools the system should track and sync.
...
// PoolRepository persists PancakeSwap V3 pool aggregates keyed by contract address.
