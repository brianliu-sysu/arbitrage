...
func (p *Pool) Ref() market.PoolRef {
	return market.PoolRefFromV4(p.ID.Hash())
}

// Apply is the sole entry point for mutating V4 pool market state from chain events.
func (p *Pool) Apply(event PoolEvent) error {
	...
	case EventKindModifyLiquidity:
		err = p.applyModifyLiquidity(event)
	default:
...
func (p *Pool) applyModifyLiquidity(event PoolEvent) error {
	if event.ModifyLiquidity == nil {
		return fmt.Errorf("modify liquidity event payload is nil")
	}
	if !p.State.IsInitialized() {
		return fmt.Errorf("pool is not initialized")
	}
	if event.ModifyLiquidity.LiquidityDelta == nil || event.ModifyLiquidity.LiquidityDelta.Sign() == 0 {
		return nil
	}
	...
	return market.ModifyLiquidity(
		...
		&p.Ticks,
		&p.Bitmap,
		event.ModifyLiquidity.TickLower,
		event.ModifyLiquidity.TickUpper,
		event.ModifyLiquidity.LiquidityDelta,
	)
}

func validateTick(index int32) error {
	if index < market.MinTick || index > market.MaxTick {
		return fmt.Errorf("tick %d out of range [%d, %d]", index, market.MinTick, market.MaxTick)
	}
	return nil
}
