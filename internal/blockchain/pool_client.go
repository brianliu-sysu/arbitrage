...
func (s *PoolClient) getRPCClient() *ethclient.Client {
	s.rpcClientMu.Lock()
...
func (s *PoolClient) rpcDialWithRetry() (*ethclient.Client, func(), error) {
...
func (s *PoolClient) resetRPCClient() {
	s.rpcClientMu.Lock()
	defer s.rpcClientMu.Unlock()
	if s.rpcClient != nil {
		s.rpcClient.Close()
...
// PoolClient 负责订阅 Uniswap V3 Pool 合约的事件日志，内置 WebSocket 断线重连。
...
func NewPoolClient(wsURL, rpcURL string, poolAddr common.Address, handler EventHandler, multicallAddr, quoterAddr common.Address, wsEnabled bool, logger logx.Logger) (*PoolClient, error) {
...
func NewSubscriber(wsURL, rpcURL string, poolAddr common.Address, handler EventHandler, multicallAddr, quoterAddr common.Address, logger logx.Logger) (*PoolClient, error) {
	return NewPoolClient(wsURL, rpcURL, poolAddr, handler, multicallAddr, quoterAddr, false, logger)
}

// Start 启动实时事件订阅（带自动重连）。
func (s *PoolClient) Start() error {
	if !s.wsEnabled {
		return nil
	}
...
func (s *PoolClient) runReconnectLoop(query ethereum.FilterQuery) {
	defer s.wg.Done()
...
func (s *PoolClient) runEventLoop(_ context.Context, client *ethclient.Client, sub ethereum.Subscription, logsCh chan types.Log) {
	defer client.Close()
	for {
		select {
		case err := <-sub.Err():
...
func (s *PoolClient) sleepOrCancel(backoff time.Duration) time.Duration {
	select {
...
func (s *PoolClient) Stop() {
	s.cancel()
	s.wg.Wait()
}

func (s *PoolClient) SyncHistorical(fromBlock, toBlock *big.Int) error {
...
func (s *PoolClient) SyncHistoricalAll(fromBlock, toBlock *big.Int) error {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return fmt.Errorf("dial for historical sync: %w", err)
	}
...
func (s *PoolClient) FetchStateViaRPC() (*PoolStateRPC, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
...
func (s *PoolClient) FetchStateViaRPCAtBlock(blockNumber uint64) (*PoolStateRPC, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchBlockNumber() (uint64, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
...
func (s *PoolClient) SimulateSwap(token0, token1 common.Address, fee uint32, amountIn *big.Int, zeroForOne bool, sqrtPriceLimitX96 *big.Int) (*big.Int, error) {
...
func (s *PoolClient) FetchPoolMetadata() (*PoolMetadata, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchTokenMetadata(tokenAddr common.Address) (*TokenMetadata, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchTickInfo(tick int32) (*TickData, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchTickBitmap(wordPos int16) (*big.Int, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchTickBitmapBatch(wordPositions []int16) (map[int16]*big.Int, error) {
...
func (s *PoolClient) FetchTickInfoBatch(ticks []int32) (map[int32]*TickData, error) {
	if len(ticks) == 0 {
		return nil, nil
	}
...
func (s *PoolClient) FetchTickBitmapAtBlock(wordPos int16, blockNumber uint64) (*big.Int, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return nil, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) FetchTickBitmapBatchAtBlock(wordPositions []int16, blockNumber uint64) (map[int16]*big.Int, error) {
	if len(wordPositions) == 0 {
		return nil, nil
	}
...
func (s *PoolClient) FetchTickInfoBatchAtBlock(ticks []int32, blockNumber uint64) (map[int32]*TickData, error) {
	if len(ticks) == 0 {
		return nil, nil
	}
...
func (s *PoolClient) FetchTickSpacing() (int32, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
		return 0, fmt.Errorf("dial: %w", err)
	}
...
func (s *PoolClient) processLog(vLog types.Log) {
...
func (s *PoolClient) markConnected() bool {
...
func (s *PoolClient) isDuplicateLog(vLog types.Log) bool {
...
func (s *PoolClient) FetchPoolFromFactory(factoryAddr common.Address, tokenA, tokenB common.Address, fee *big.Int) (common.Address, error) {
	client, done, err := s.rpcDialWithRetry()
	if err != nil {
