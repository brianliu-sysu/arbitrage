...
type AppService struct {
	...
	quotes    *quotepancakev3.QuoteService
...
// Quote executes the PancakeSwap V3 quote use case for the given request.
...
func (s *AppService) quoteSinglePool(ctx context.Context, req Request, poolAddress common.Address) (Response, error) {
	...
	route := quotepancakev3.NewDirectRoute(poolAddress, req.TokenIn, req.TokenOut)
	return Response{
		TokenIn:   req.TokenIn,
...
func (s *AppService) quoteBestRoute(ctx context.Context, req Request) (Response, error) {
	...
	routeService := quotepancakev3.NewRouteService(graph, s.maxHops)
...
func (s *AppService) buildPoolGraph(ctx context.Context) (quotepancakev3.PoolGraph, error) {
	if s.registry == nil {
	...
	edges := make([]quotepancakev3.PoolEdge, 0, len(addresses))
	for _, address := range addresses {
		...
		edges = append(edges, quotepancakev3.PoolEdge{
			PoolAddress: pool.Address,
			Token0:      pool.Token0,
			Token1:      pool.Token1,
		})
	}
	return quotepancakev3.NewStaticPoolGraph(edges), nil
}

func (s *AppService) loadRoutePools(ctx context.Context, route quotepancakev3.Route) (map[common.Address]*marketpancake.Pool, error) {
	pools := make(map[common.Address]*marketpancake.Pool, route.Len())
...
func (s *AppService) ensureRouteReady(route quotepancakev3.Route) error {
	if s.readiness == nil {
		return nil
	}
	for _, hop := range route.Hops {
