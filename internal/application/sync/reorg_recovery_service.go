...
type ReorgRecoveryHooks[PoolID comparable, Event any, Pool any] struct {
	...
	NotifyRecovered      func(context.Context, uint64, []PoolID) error
}
...
func (s *ReorgRecoveryService[PoolID, Event, Pool]) Recover(ctx context.Context, reorg blockchain.Reorg, poolIDs []PoolID) error {
	for _, poolID := range poolIDs {
		...
		if fromBlock <= toBlock {
			...
			for blockNumber := fromBlock; blockNumber <= toBlock; blockNumber++ {
				...
				if err := s.hooks.ApplyBlock(ctx, blockNumber, header.Hash, eventsByBlock[blockNumber], []PoolID{poolID}, true); err != nil {
	...
	if s.hooks.NotifyRecovered != nil {
		if err := s.hooks.NotifyRecovered(ctx, reorg.RemoteHead.Number, poolIDs); err != nil {
			return fmt.Errorf("notify reorg recovery at block %d: %w", reorg.RemoteHead.Number, err)
		}
	}
	return nil
}
