...
type memorySnapshotRepo struct {
	latest map[common.Address]*marketclv3.Snapshot
}
...
func TestRestorePoolSkipsOlderSnapshot(t *testing.T) {
	...
	repo := &memorySnapshotRepo{latest: make(map[common.Address]*marketclv3.Snapshot)}
	olderPool := marketclv3.NewPool(address, common.Address{}, common.Address{}, 3000, 60)
	olderPool.State.SqrtPriceX96 = sqrtPrice
	olderPool.State.Tick = 0
	olderPool.Ticks.GetOrCreate(-120).LiquidityGross = big.NewInt(1)
	repo.latest[address] = marketclv3.NewSnapshot(olderPool, 100, time.Unix(0, 0).UTC())
...
func TestRestorePoolAppliesNewerSnapshot(t *testing.T) {
	...
	repo := &memorySnapshotRepo{latest: make(map[common.Address]*marketclv3.Snapshot)}
	newerPool := marketclv3.NewPool(address, common.Address{}, common.Address{}, 3000, 60)
	...
	repo.latest[address] = marketclv3.NewSnapshot(newerPool, 200, time.Unix(0, 0).UTC())
