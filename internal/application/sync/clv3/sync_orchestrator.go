...
func NewServices(deps ServiceDeps) *Services {
	...
	admission := NewPoolAdmissionService(deps.Registry, bootstrap, readiness)
	blockApply := NewBlockApplyService(deps.Pools, deps.Checkpoints, snapshots, readiness, deps.Listener)
	catchup := NewCatchupService(deps.Config, deps.Pools, deps.Checkpoints, deps.Fetcher, deps.Parser, blockApply, admission, deps.Blocks)
	reorg := NewReorgRecoveryService(deps.Pools, deps.Bootstrap, snapshots, blockApply, readiness)
	blockConsumer := NewBlockConsumer(deps.Parser, blockApply, admission, reorg)
