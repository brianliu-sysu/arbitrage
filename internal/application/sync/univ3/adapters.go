...
func (a *poolRepositoryAdapter) Save(ctx context.Context, pool *marketclv3.Pool) error {
	return a.inner.Save(ctx, &marketv3.Pool{Pool: *pool})
}
...
type snapshotRepositoryAdapter struct {
	inner marketv3.SnapshotRepository
}
...
func newUniswapPool(address, token0, token1 common.Address, fee uint32, tickSpacing int32) *marketclv3.Pool {
	return &marketv3.NewPool(address, token0, token1, fee, tickSpacing).Pool
}

func adaptUniswapDeps(deps ServiceDeps) clv3sync.ServiceDeps {
