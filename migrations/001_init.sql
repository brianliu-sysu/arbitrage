-- Initial persistence schema for Uniswap V3 sync state.
...
CREATE TABLE IF NOT EXISTS pool_ticks (
    pool_address BYTEA NOT NULL REFERENCES pools(address) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS pool_tick_bitmap (
    pool_address BYTEA NOT NULL REFERENCES pools(address) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS checkpoints (
    pool_address BYTEA PRIMARY KEY REFERENCES pools(address) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS snapshot_ticks (
    ...
    FOREIGN KEY (pool_address, block_number) REFERENCES snapshots(pool_address, block_number) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS snapshot_tick_bitmap (
    pool_address BYTEA NOT NULL,
    block_number BIGINT NOT NULL,
    word_pos SMALLINT NOT NULL,
    word_value NUMERIC NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_snapshots_pool_block ON snapshots(pool_address, block_number DESC);
